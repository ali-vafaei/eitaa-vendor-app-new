// src/theme/theme-options.ts (نسخه کامل و نهایی با تمام اصلاحات)
...
// ||||||||||||||||||||||||||||| اصلاح نهایی |||||||||||||||||||||||||||||
// ----------------------------------------------------------------------
const themeOptions = (pathname: string | null) => { // ۱. پذیرش مقدار null برای pathname
  let themeOption;
  ...
  // ۲. بررسی می‌کنیم که آیا pathname یک رشته معتبر است یا نه
  // این کار از بروز خطا در سمت سرور (جایی که pathname وجود ندارد) جلوگیری می‌کند
  if (typeof pathname === 'string') {
    ...
    // ۳. اگر pathname معتبر نبود، از تم پیش‌فرض استفاده می‌کنیم
    themeOption = themesOptionList[THEMES.DEFAULT];
  }
